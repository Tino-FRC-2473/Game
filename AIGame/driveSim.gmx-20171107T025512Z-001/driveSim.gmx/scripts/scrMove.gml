...
while (abs(totalDXPos) < abs(maxDXPos) && !place_meeting(tryXPos + xStep, tryYPos, objBlock)) {
	tryXPos += xStep;
	totalDXPos += xStep;
}

while (abs(totalDYPos) < abs(maxDYPos) && !place_meeting(tryXPos, tryYPos - yStep, objBlock)) {
	tryYPos -= yStep;
	totalDYPos -= yStep;
}
